...
async def cancel_chat(session_id: str, user_id: str = Depends(get_current_user)):
    """
...
async def magic(request: Request, user_id: str = Depends(get_current_user)):
    ...
    """
    data = await request.json()
    await handle_magic(data, user_id=user_id)
    return {"status": "done"}


@router.post("/magic/cancel/{session_id}")
async def cancel_magic(session_id: str, user_id: str = Depends(get_current_user)) -> Dict[str, str]:
    """
